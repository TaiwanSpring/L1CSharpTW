...
//	import static l1j.server.server.model.skill.L1SkillId.WIND_SHACKLE;
	...
	using L1HateList = LineageServer.Server.Server.Model.L1HateList;
	using L1Inventory = LineageServer.Server.Server.Model.L1Inventory;
	using L1Magic = LineageServer.Server.Server.Model.L1Magic;
	using L1MobGroupInfo = LineageServer.Server.Server.Model.L1MobGroupInfo;
	using L1MobSkillUse = LineageServer.Server.Server.Model.L1MobSkillUse;
	using L1NpcChatTimer = LineageServer.Server.Server.Model.L1NpcChatTimer;
	...
	using S_MoveCharPacket = LineageServer.Server.Server.serverpackets.S_MoveCharPacket;
	...
	using TimerPool = LineageServer.Server.Server.utils.TimerPool;
	using Lists = LineageServer.Server.Server.utils.collections.Lists;
	using Maps = LineageServer.Server.Server.utils.collections.Maps;

	[Serializable]
	public class L1NpcInstance : L1Character
	{
...
//JAVA TO C# CONVERTER WARNING: The .NET Type.FullName property will not always yield results identical to the Java Class.getName method:
		private static Logger _log = Logger.getLogger(typeof(L1NpcInstance).FullName);
		...
		internal class NpcAITimerImpl : TimerTask, NpcAI
		{
			...
			private class DeathSyncTimer : TimerTask
			{
				...
				{
					_timerPool.Timer.schedule(new DeathSyncTimer(outerInstance), delay);
				}
			...
			public virtual void start()
			{
				...
				_timerPool.Timer.schedule(NpcAITimerImpl.this, 0);
			}

			internal virtual void stop()
			{
				outerInstance.mobSkill.resetAllSkillUseCount();
				_timerPool.Timer.schedule(new DeathSyncTimer(this), 0); // 死亡同期を開始
			}

			// 同じインスタンスをTimerへ登録できない為、苦肉の策。
			internal virtual void schedule(int delay)
			{
				_timerPool.Timer.schedule(new NpcAITimerImpl(outerInstance), delay);
			}
		...
		internal class NpcAIThreadImpl : IRunnableStart, NpcAI
		{
			private readonly L1NpcInstance outerInstance;
		...
		public virtual void attackTarget(L1Character target)
		{
			...
			if (attack.calcHit())
			{
				if (target.hasSkillEffect(COUNTER_BARRIER))
				{
					...
					bool isProbability = magic.calcProbabilityMagic(COUNTER_BARRIER);
		...
		{
			IList<L1GroundInventory> gInventorys = Lists.newList();
		...
		protected internal IList<L1ItemInstance> _targetItemList = Lists.newList(); // ダーゲットアイテム一覧
		...
		public void startHpRegeneration()
		{
			...
			if (!_hprRunning && (hprInterval > 0) && (hpr > 0))
			{
				...
				L1NpcRegenerationTimer.Instance.scheduleAtFixedRate(_hprTimer, hprInterval, hprInterval);
				_hprRunning = true;
			}
		}
		...
		public void startMpRegeneration()
		{
			...
			if (!_mprRunning && (mprInterval > 0) && (mpr > 0))
			{
				...
				L1NpcRegenerationTimer.Instance.scheduleAtFixedRate(_mprTimer, mprInterval, mprInterval);
				_mprRunning = true;
			}
		}
		...
		internal class MprTimer : TimerTask
		{
			...
			public override void run()
			{
				try
				{
					if ((!outerInstance._destroyed && !outerInstance.Dead) && ((outerInstance.CurrentHp > 0) && (outerInstance.CurrentMp < outerInstance.MaxMp)))
					{
		...
		internal class DigestItemTimer : IRunnableStart
		{
			private readonly L1NpcInstance outerInstance;
			...
			public override void run()
			{
				...
				while (!outerInstance._destroyed && (outerInstance._digestItems.Count > 0))
				{
					...
					object[] keys = outerInstance._digestItems.Keys.ToArray();
					foreach (object key2 in keys)
					{
						...
						if (digestCounter <= 0)
						{
							...
							L1ItemInstance digestItem = outerInstance.Inventory.getItem(key.Value);
							if (digestItem != null)
							{
						...
						{
							outerInstance._digestItems[key] = digestCounter.Value;
						}
					}
				}
		...
		{
			if (pc.hasSkillEffect(L1SkillId.60) || pc.hasSkillEffect(L1SkillId.97))
		...
		{ // 怪物解除遁地、飛天、冰凍
			if (HiddenStatus == HIDDEN_STATUS_SINK)
			{
				...
				if (!pc.hasSkillEffect(L1SkillId.60) && !pc.hasSkillEffect(L1SkillId.97) && !pc.Gm)
				{
					_hateList.add(pc, 0);
					_target = pc;
				}
