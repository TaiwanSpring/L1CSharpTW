...
namespace LineageServer.Server.Server.Model
{
	...
	{
		private static readonly int[] TELEPORT_LOC_MAPID = new int[] {4, 4, 4, 350};

		private static readonly Point[] TELEPORT_LOC_GIRAN = new Point[]
		{
			new Point(33419, 32810),
			new Point(33343, 32723),
			new Point(33553, 32712),
			new Point(32702, 32842)
		};

		private static readonly Point[] TELEPORT_LOC_HEINE = new Point[]
		{
			new Point(33604, 33236),
			new Point(33649, 33413),
			new Point(33553, 32712),
			new Point(32702, 32842)
		};

		private static readonly Point[] TELEPORT_LOC_ADEN = new Point[]
		{
			new Point(33966, 33253),
			new Point(33921, 33177),
			new Point(33553, 32712),
			new Point(32702, 32842)
		};

		private static readonly Point[] TELEPORT_LOC_GLUDIN = new Point[]
		{
			new Point(32628, 32807),
			new Point(32623, 32729),
			new Point(33553, 32712),
			new Point(32702, 32842)
		};

		// アジトID一覧
		private static readonly IList<int> _houseIds = HouseTable.HouseIdList;
		...
		/// <param name="cha">
		/// @return </param>
		public static bool isInHouseLoc(int houseId, int locx, int locy, short mapid)
		{
			bool ret = false;
			if (houseId == 262145)
			{ // ギランアジト1
		...
		/// <param name="cha">
		/// @return </param>
		public static bool isInHouse(int locx, int locy, short mapid)
		{
			bool result = false;
			foreach (int? houseId in _houseIds)
			{
				if (isInHouseLoc(houseId.Value, locx, locy, mapid))
				{
					result = true;
					break;
				}
		...
		{ // houseIdからテレポート先の座標を返す
			int[] loc = new int[3];
			if ((houseId >= 262145) && (houseId <= 262189))
			{ // ギランアジト
				loc[0] = TELEPORT_LOC_GIRAN[number].X;
				loc[1] = TELEPORT_LOC_GIRAN[number].Y;
				loc[2] = TELEPORT_LOC_MAPID[number];
			}
			else if ((houseId >= 327681) && (houseId <= 327691))
			{ // ハイネアジト
				loc[0] = TELEPORT_LOC_HEINE[number].X;
				loc[1] = TELEPORT_LOC_HEINE[number].Y;
				loc[2] = TELEPORT_LOC_MAPID[number];
			}
			else if ((houseId >= 458753) && (houseId <= 458819))
			{ // アデンアジト
				loc[0] = TELEPORT_LOC_ADEN[number].X;
				loc[1] = TELEPORT_LOC_ADEN[number].Y;
				loc[2] = TELEPORT_LOC_MAPID[number];
			}
			else if ((houseId >= 524289) && (houseId <= 524294))
			{ // グルーディンアジト1~6
				loc[0] = TELEPORT_LOC_GLUDIN[number].X;
				loc[1] = TELEPORT_LOC_GLUDIN[number].Y;
				loc[2] = TELEPORT_LOC_MAPID[number];
			}
			return loc;
		}
