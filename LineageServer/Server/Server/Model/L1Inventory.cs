...
{
    [Serializable]
    class L1Inventory : L1Object
    {

        private const long serialVersionUID = 1L;

        protected internal IList<L1ItemInstance> _items = Lists.newConcurrentList<L1ItemInstance>();
        ...
        {
            IList<L1ItemInstance> itemList = Lists.newList<L1ItemInstance>();
            foreach (L1ItemInstance item in _items)
            {
                if (item.ItemId == id)
                {
