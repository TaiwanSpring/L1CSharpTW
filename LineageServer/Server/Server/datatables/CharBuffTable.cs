...
namespace LineageServer.Server.Server.datatables
{
	...
	using Effect = LineageServer.Server.Server.Model.item.action.Effect;
...
//JAVA TO C# CONVERTER WARNING: The .NET Type.FullName property will not always yield results identical to the Java Class.getName method:
		private static Logger _log = Logger.getLogger(typeof(CharBuffTable).FullName);
				...
				while (rs.next())
				{
					...
					switch (skillid)
					{
						...
						case EFFECT_BLESS_OF_SAELL:
							...
							break;
						default:
							if (skillid >= EFFECT_MAGIC_STONE_A_1 && skillid <= EFFECT_MAGIC_STONE_D_9)
							{ // 附魔石
								remaining_time = remaining_time / 32;
