...
namespace LineageServer.Server.DataTables
{
	...
	using L1Drop = LineageServer.Server.Templates.L1Drop;
...
//JAVA TO C# CONVERTER WARNING: The .NET Type.FullName property will not always yield results identical to the Java Class.getName method:
		...
		{
			IDictionary<int, IList<L1Drop>> droplistMap = MapFactory.newMap();
		...
		public virtual void dropShare(L1NpcInstance npc, IList<L1Character> acquisitorList, IList<int> hateList)
		{
			...
			for (int i = inventory.Size; i > 0; i--)
			{
				...
				{ // ノンオートルーティング
					IList<int> dirList = ListFactory.newList();
					for (int j = 0; j < 8; j++)
					{
						dirList.Add(j);
					}
