...
namespace LineageServer.Server.Model.Instance
{
	class L1NpcInstance : L1Character
	{
		...
		{
			IList<GameObject> targetKnownObjects = targetPlayer.KnownObjects;
		...
		public static void shuffle(GameObject[] arr)
		{
		...
		protected internal IList<L1ItemInstance> _targetItemList = ListFactory.NewList<L1ItemInstance>(); // ダーゲットアイテム一覧
		...
		public virtual void setting_template(L1Npc template)
		{
			...
			{
				_digestItems = MapFactory.NewMap<int, int>();
			}
		...
		{
			if (factoryMapping.ContainsKey(l1Npc.Impl))
			{
				return factoryMapping[l1Npc.Impl].Invoke(l1Npc);
			}
			else
			{
				return null;
		...
		public static void RegisterNpcInstance(string impName, Func<L1Npc, L1NpcInstance> ctor)
		{
			if (factoryMapping.ContainsKey(impName))
			{
				Debug.Fail("wtf");
			}
			else
			{
				factoryMapping.Add(impName, ctor);
			}
		}
	}
