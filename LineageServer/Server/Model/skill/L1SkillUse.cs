        ...
        private bool NormalSkillUsable
        {
            get
            {
                ...
                if (_user is L1PcInstance)
                {
                    ...
                    if ((pc.Inventory as L1PcInventory).Weight242 >= 197)
                    { // \f1你攜帶太多物品，因此無法使用法術。
        ...
        private bool HPMPConsume
        {
            get
            {
                ...
                else
                {
                    ...
                    L1PcInventory pcInventory = (_player.Inventory as L1PcInventory);
                    // 裝備MP減免 一次只需判斷一個 
                    if ((_skillId == L1SkillId.PHYSICAL_ENCHANT_DEX) && pcInventory.checkEquipped(20013))
                    { // 敏捷魔法頭盔使用通暢氣脈術
                        _mpConsume /= 2;
                    }
                    else if ((_skillId == L1SkillId.HASTE) && pcInventory.checkEquipped(20013))
                    { // 敏捷魔法頭盔使用加速術
                        _mpConsume /= 2;
                    }
                    else if ((_skillId == L1SkillId.HEAL) && pcInventory.checkEquipped(20014))
                    { // 治癒魔法頭盔使用初級治癒術
                        _mpConsume /= 2;
                    }
                    else if ((_skillId == L1SkillId.EXTRA_HEAL) && pcInventory.checkEquipped(20014))
                    { // 治癒魔法頭盔使用中級治癒術
                        _mpConsume /= 2;
                    }
                    else if ((_skillId == L1SkillId.ENCHANT_WEAPON) && pcInventory.checkEquipped(20015))
                    { // 力量魔法頭盔使用擬似魔法武器
                        _mpConsume /= 2;
                    }
                    else if ((_skillId == L1SkillId.DETECTION) && pcInventory.checkEquipped(20015))
                    { // 力量魔法頭盔使用無所遁形術
                        _mpConsume /= 2;
                    }
                    else if ((_skillId == L1SkillId.PHYSICAL_ENCHANT_STR) && pcInventory.checkEquipped(20015))
                    { // 力量魔法頭盔使用體魄強健術
                        _mpConsume /= 2;
                    }
                    else if ((_skillId == L1SkillId.HASTE) && pcInventory.checkEquipped(20008))
                    { // 小型風之頭盔使用加速術
                        _mpConsume /= 2;
                    }
                    else if ((_skillId == L1SkillId.HASTE) && pcInventory.checkEquipped(20023))
                    { // 風之頭盔使用加速術
                        _mpConsume = 25;
                    }
                    else if ((_skillId == L1SkillId.GREATER_HASTE) && pcInventory.checkEquipped(20023))
                    { // 風之頭盔使用強力加速術
                        _mpConsume /= 2;
                    }
