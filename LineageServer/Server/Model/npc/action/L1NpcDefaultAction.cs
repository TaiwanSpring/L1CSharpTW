...
namespace LineageServer.Server.Model.npc.action
{
	public class L1NpcDefaultAction
	{
		private class Action
		{
			...
			internal int specialAttack = 0; // 特殊攻擊
			internal int rangedAttack = 1; // 遠距攻擊
			internal int status = 0; // 狀態
		}

		private static readonly IDictionary<int, Action> _dataMap = MapFactory.NewMap<int, Action>();
		...
		public static L1NpcDefaultAction Instance
		{
			get
			{
				return _instance;
		...
		public virtual int getDefaultAttack(int gfxid)
		{
			if (_dataMap.ContainsKey(gfxid))
			{
				return _dataMap[gfxid].defaultAttack;
			}
			return 1;
		}

		public virtual int getSpecialAttack(int gfxid)
		{
			if (_dataMap.ContainsKey(gfxid))
			{
				return _dataMap[gfxid].specialAttack;
			}
			return 0;
		}

		public virtual int getRangedAttack(int gfxid)
		{
			if (_dataMap.ContainsKey(gfxid))
			{
				return _dataMap[gfxid].rangedAttack;
			}
			return 1;
		}

		public virtual int getStatus(int gfxid)
		{
			if (_dataMap.ContainsKey(gfxid))
			{
				return _dataMap[gfxid].status;
			}
			return 0;
		}
	}
