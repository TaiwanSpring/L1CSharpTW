    ...
    class L1Magic
    {
        ...
        public virtual bool calcProbabilityMagic(int skillId)
        {
            ...
            if ((_calcType == PC_PC) || (_calcType == NPC_PC))
            {
                ...
                {
                    if ((skillId != L1SkillId.WEAPON_BREAK) && (skillId != L1SkillId.CANCELLATION))
                    {
                        return false;
                    }
                }
            ...
            if ((_pc != null) && (_targetPc != null))
            {
                ...
                { // セーフティーゾーン
                    if ((skillId == L1SkillId.WEAPON_BREAK) ||
                        ...
                        (skillId == L1SkillId.CURSE_PARALYZE) ||
                        (skillId == L1SkillId.MANA_DRAIN) ||
                        (skillId == L1SkillId.DARKNESS) ||
                        (skillId == L1SkillId.WEAKNESS) ||
                        (skillId == L1SkillId.DISEASE) ||
                        (skillId == L1SkillId.DECAY_POTION) ||
                        (skillId == L1SkillId.MASS_SLOW) ||
                        (skillId == L1SkillId.ENTANGLE) ||
                        (skillId == L1SkillId.ERASE_MAGIC) ||
                        (skillId == L1SkillId.EARTH_BIND) ||
                        (skillId == L1SkillId.AREA_OF_SILENCE) ||
                        (skillId == L1SkillId.WIND_SHACKLE) ||
                        (skillId == L1SkillId.STRIKER_GALE) ||
                        (skillId == L1SkillId.SHOCK_STUN) ||
                        (skillId == L1SkillId.FOG_OF_SLEEPING) ||
                        (skillId == L1SkillId.ICE_LANCE) ||
                        ...
                        (skillId == L1SkillId.RETURN_TO_NATURE) ||
                        (skillId == L1SkillId.ICE_LANCE_COCKATRICE) ||
                        (skillId == L1SkillId.ICE_LANCE_BASILISK))
                    {
                        return false;
                    }
                }
        ...
        private int calcProbability(int skillId)
        {
            ...
            else
            {
                ...
                if (skillId == L1SkillId.RETURN_TO_NATURE)
                {
            ...
            if ((skillId == L1SkillId.ELEMENTAL_FALL_DOWN) || (skillId == L1SkillId.RETURN_TO_NATURE) || (skillId == L1SkillId.ENTANGLE) || (skillId == L1SkillId.ERASE_MAGIC) || (skillId == L1SkillId.AREA_OF_SILENCE) || (skillId == L1SkillId.WIND_SHACKLE) || (skillId == L1SkillId.STRIKER_GALE) || (skillId == L1SkillId.POLLUTE_WATER) || (skillId == L1SkillId.EARTH_BIND))
            {
            ...
            else if (skillId == L1SkillId.COUNTER_BARRIER)
            {
                // 成功確率は 基本確率 + LV差1毎に+-1%
            ...
            else if ((skillId == L1SkillId.GUARD_BRAKE) || (skillId == L1SkillId.RESIST_FEAR) || (skillId == L1SkillId.HORROR_OF_DEATH))
            {
            ...
            // 状態異常に対する耐性
            if (skillId == L1SkillId.EARTH_BIND)
            {
                if ((_calcType == PC_PC) || (_calcType == NPC_PC))
                {
            ...
            else if ((skillId == L1SkillId.CURSE_BLIND) || (skillId == L1SkillId.DARKNESS) || (skillId == L1SkillId.DARK_BLIND))
            {
                if ((_calcType == PC_PC) || (_calcType == NPC_PC))
                {
        ...
        // 擁有這些狀態的, 不會受到傷害(無敵)
        private static readonly int[] INVINCIBLE = new int[]
        {
            L1SkillId.ABSOLUTE_BARRIER, L1SkillId.ICE_LANCE,
            L1SkillId.FREEZING_BLIZZARD, L1SkillId.FREEZING_BREATH,
            L1SkillId.EARTH_BIND, L1SkillId.ICE_LANCE_COCKATRICE,
            L1SkillId.ICE_LANCE_BASILISK
        };
        ...
        public virtual int calcMagicDamage(int skillId)
        {
            ...
            if (skillId != L1SkillId.JOY_OF_PAIN)
            { // 疼痛的歡愉無視魔免
        ...
        public virtual int calcPcFireWallDamage()
        {
            ...
            L1Skills l1skills = SkillsTable.Instance.getTemplate(L1SkillId.FIRE_WALL);
        ...
        private int calcPcMagicDamage(int skillId)
        {
            ...
            if (skillId == L1SkillId.MIND_BREAK)
            {
        ...
        private int calcMrDefense(int dmg)
        {
            ...
            if ((_calcType == PC_PC) || (_calcType == PC_NPC))
            {
                if (mr <= 100)
                {
                    mrFloor = Math.Floor((double)(mr - _pc.OriginalMagicHit) / 2);
                }
                else if (mr >= 100)
                {
                    mrFloor = Math.Floor((double)(mr - _pc.OriginalMagicHit) / 10);
                }
