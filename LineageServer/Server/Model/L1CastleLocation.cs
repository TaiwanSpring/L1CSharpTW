...
namespace LineageServer.Server.Model
{
    class L1CastleLocation
    {
        ...
        private static readonly IDictionary<int, L1Location> _towers = MapFactory.NewMap<int, L1Location>();
        ...
        private static readonly IDictionary<int, L1MapArea> _areas = MapFactory.NewMap<int, L1MapArea>();


        private static readonly IDictionary<int, int> _innerTowerMaps = MapFactory.NewMap<int, int>();


        private static readonly IDictionary<int, L1Location> _subTowers = MapFactory.NewMap<int, L1Location>();
        ...
        {
            foreach (KeyValuePair<int, L1Location> entry in _towers)
            {
                if (entry.Value == loc)
                {
                    return entry.Key;
        ...
        {
            foreach (KeyValuePair<int, L1MapArea> entry in _areas)
            {
            ...
            foreach (KeyValuePair<int, int> entry in _innerTowerMaps)
            {
        ...
        private static IDictionary<int, int> _castleTaxRate = MapFactory.NewMap<int, int>();
        ...
        public static void setCastleTaxRate()
        {
            ...
            if (_listener == null)
            {
                ...
                Container.Instance.Resolve<IGameTimeClock>().AddListener(_listener);
            }
        }
